...
sudo cmake --install --prefix $macos_install_root

strip -S "$macos_install_root"/libSDL3.dylib
cp "$macos_install_root"/libSDL3.dylib $macos_bin_root/
...
cp "$macos_install_root"/libSDL3_image.dylib $macos_bin_root/

# SDL_mixer
cmake -S SDL_mixer sdl_mixer-build-release "${sdl_mixer_flags[@]}"
cmake --build sdl_mixer-build-release --config Release

strip -S "$macos_install_root"/libSDL3_mixer.dylib
cp "$macos_install_root"/libSDL3_mixer.dylib $macos_bin_root/

# SDL_ttf
cmake -S SDL_ttf sdl_ttf-build-release "${sdl_ttf_flags[@]}"
cmake --build sdl_ttf-build-release --config Release

strip -S "$macos_install_root"/libSDL3_ttf.dylib
cp "$macos_install_root"/libSDL3_ttf.dylib $macos_bin_root/
...
strip -S "$macos_install_root"/libSDL3_shadercross.dylib
cp "$macos_install_root"/libSDL3_shadercross.dylib $macos_bin_root/

strip -S "$macos_install_root"/libdxcompiler.dylib
cp "$macos_install_root"/libdxcompiler.dylib $macos_bin_root/

strip -S "$macos_install_root"/libdxcompiler.dylib
cp "$macos_install_root"/libdxcompiler.dylib $macos_bin_root/

strip -S "$macos_install_root"/libdxil.dylib
cp "$macos_install_root"/libdxil.dylib $macos_bin_root/
