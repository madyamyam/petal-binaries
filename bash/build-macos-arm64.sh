...
yes | brew install nasm plutosvg webp libvorbis opus opusfile flac bash
...
echo $GH_PASSWD | sudo cmake --build sdl-build-release --target install --config Release # bypasses installation privileges issue
...
strip -S sdl_image-build-release/libSDL3_image.dylib
cp sdl_image-build-release/libSDL3_image.dylib $macos_bin_root/
...
strip -S sdl_mixer-build-release/libSDL3_mixer.dylib
cp sdl_mixer-build-release/libSDL3_mixer.dylib $macos_bin_root/
